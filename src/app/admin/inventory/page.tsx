...
function InventoryContent() {
  ...
  const adjustProductStock = async (productId: string, currentStock: number, delta: number) => {
    ...
    try {
      // Try to update Firestore; if product is mock-only (from static list), create it
      const docRef = doc(db, 'products', productId);
      try {
        await updateDoc(docRef, { stockCount: newStock, updatedAt: Timestamp.now() });
      } catch {
        // Document may not exist yet (mock data) — create it with setDoc
